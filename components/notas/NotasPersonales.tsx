...
export function NotasPersonales({ notas }: { notas: NotaPersonal[] }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-col gap-2 rounded-[16px] bg-tarjeta p-3.5 shadow-sm">
        <textarea
          value={texto}
          onChange={(e) => setTexto(e.target.value)}
          rows={2}
          placeholder="Escribí una nota para vos…"
          className="max-h-40 min-h-[56px] resize-none rounded-[12px] border border-campo px-3 py-2 text-[14px] outline-none focus:border-azul"
        />
        ...
        <button
          ...
          className="self-end btn-primario px-5 py-2 text-[13px] font-bold text-white disabled:opacity-40"
        >
          Guardar nota
        </button>
      </div>
      ...
      {notas.length === 0 ? (
        ...
        <ul className="flex flex-col gap-2">
          {notas.map((n) => (
            <li key={n.id} className="rounded-[14px] bg-tarjeta px-3.5 py-3 shadow-sm">
              <p className="text-[14px] leading-snug whitespace-pre-wrap text-tinta">{n.cuerpo}</p>
              <div className="mt-1.5 flex items-center justify-between">
                <span className="text-[11px] font-medium text-tenue-2">{fechaCorta(n.creado_en)}</span>
                <button
                  type="button"
                  onClick={() => borrar(n.id)}
                  disabled={pendiente}
