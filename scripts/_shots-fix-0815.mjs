...
try {
  for (const [nombre, vp] of [
    ...
    await page.waitForTimeout(7000); // realtime/SW: networkidle no llega
    await page.screenshot({ path: `${OUT}/fix-${nombre}-arriba.png` });
    ...
    await page.screenshot({ path: `${OUT}/fix-${nombre}-catalogo.png` });
    // Abrir el primer producto (detalle).
    const prod = page.locator("#catalogo ~ * button, [id='catalogo'] ~ * a").first();
    try {
      await page.mouse.click(vp.width / 2, vp.height / 2);
      await page.waitForTimeout(2000);
      await page.screenshot({ path: `${OUT}/fix-${nombre}-detalle.png` });
    } catch {}
    await ctx.close();
  }
