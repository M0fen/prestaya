// Bandeja de anulaciones de pago (gestores). El supervisor pide anular pagos de
// su zona y — desde 08-05 — el COBRADOR pide corregir cobros PROPIOS mal
// cargados. Acá una SEGUNDA persona (admin u otro gestor distinto de quien
// pidió) confirma o rechaza. Doble registro anti-fraude sobre el dinero.
import { requireGestor } from "@/lib/auth";
import { createSupabaseServer } from "@/lib/supabase/server";
import { getSolicitudesPendientes } from "@/lib/data/anulaciones";
import { alcanceDelActor } from "@/lib/data/alcance";
import { SolicitudesAnulacion } from "@/components/admin/SolicitudesAnulacion";

export const dynamic = "force-dynamic";

export default async function AnulacionesPage() {
  const usuario = await requireGestor();
  const db = await createSupabaseServer();
  // Acota la bandeja a la zona del gestor (admin = global): sin esto un supervisor
  // veía el motivo/solicitante de anulaciones de otras zonas (RLS de la tabla ancha).
  const alcance = await alcanceDelActor();
  const solicitudes = await getSolicitudesPendientes(db, alcance);

  return (
    <div className="mx-auto flex max-w-[720px] flex-col gap-5">
      <div className="flex flex-col gap-0.5">
        <h1 className="text-[24px] font-extrabold tracking-[-0.02em] text-tinta">
          Anulaciones y correcciones
        </h1>
        <span className="text-[13px] font-medium text-gris">
          Pedidos de anular pagos: los del supervisor y las <b>correcciones que reportan los
          cobradores</b> sobre sus propios cobros. Los confirma una segunda persona (no quien
          los pidió): doble registro sobre el dinero. El pago se marca anulado, nunca se borra.
        </span>
      </div>

      <SolicitudesAnulacion solicitudes={solicitudes} yoId={usuario.id} />
    </div>
  );
}
